...
void MainWindow::buildControlBar(QWidget *parent) {
    ...
    // Click anywhere → instant seek (ClickableSlider emits sliderMoved on click)
    connect(m_seekSlider, &QSlider::sliderMoved, [this](int val){
        qint64 dur = m_engine->duration();
        if (dur > 0) m_engine->seek(dur * val / 1000);
    });
